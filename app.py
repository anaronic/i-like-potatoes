...
if st.button("Generate a cute reel"):
    # Validation for exactly 4 images in each set
    if len(set_a) != 4 or len(set_b) != 4:
        ...
        with st.spinner("Generating..."):
            ...
            # Combine the sets: Set A (Photos 1-4) then Set B (Photos 5-8)
            all_uploaded_files = set_a + set_b
            img_paths = []
            
            for i, f in enumerate(all_uploaded_files):
